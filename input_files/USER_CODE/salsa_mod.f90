...
!
!-- SALSA variables:
    CHARACTER(LEN=20)  ::  bc_aer_b = 'neumann'                   !< bottom boundary condition
    CHARACTER(LEN=20)  ::  bc_aer_l = 'undefined'                 !< left boundary condition
    CHARACTER(LEN=20)  ::  bc_aer_n = 'undefined'                 !< north boundary condition
    CHARACTER(LEN=20)  ::  bc_aer_r = 'undefined'                 !< right boundary condition
    CHARACTER(LEN=20)  ::  bc_aer_s = 'undefined'                 !< south boundary condition
    CHARACTER(LEN=20)  ::  bc_aer_t = 'neumann'                   !< top boundary condition
    ...
    INTEGER(iwp) ::  communicator_salsa     !< stores the number of the MPI communicator to be used
    ...
    INTEGER(iwp) ::  ibc_aer_b              !< index for the bottom boundary condition
    INTEGER(iwp) ::  ibc_aer_t              !< index for the top boundary condition
...
!-- SALSA switches:
    ...
    LOGICAL ::  bc_dirichlet_aer_l      = .FALSE.  !< flag for indicating a dirichlet condition at
                                                   !< the left boundary
    LOGICAL ::  bc_dirichlet_aer_n      = .FALSE.  !< flag for indicating a dirichlet condition at
                                                   !< the north boundary
    LOGICAL ::  bc_dirichlet_aer_r      = .FALSE.  !< flag for indicating a dirichlet condition at
                                                   !< the right boundary
    LOGICAL ::  bc_dirichlet_aer_s      = .FALSE.  !< flag for indicating a dirichlet condition at
                                                   !< the south boundary
    LOGICAL ::  bc_radiation_aer_l      = .FALSE.  !< flag for indicating a radiation/neumann
                                                   !< condition at the left boundary
    LOGICAL ::  bc_radiation_aer_n      = .FALSE.  !< flag for indicating a radiation/neumann
                                                   !< condition at the north boundary
    LOGICAL ::  bc_radiation_aer_r      = .FALSE.  !< flag for indicating a radiation/neumann
                                                   !< condition at the right boundary
    LOGICAL ::  bc_radiation_aer_s      = .FALSE.  !< flag for indicating a radiation/neumann
...
!-- Aerosol emission data attributes
    TYPE salsa_emission_attribute_type
       ...
       INTEGER(iwp) ::  tind_p             !< time index for following time in salsa emission data
...
!-- Set bottom boundary condition flag
    IF ( bc_aer_b == 'dirichlet' )  THEN
       ibc_aer_b = 0
    ELSEIF ( bc_aer_b == 'neumann' )  THEN
       ibc_aer_b = 1
    ELSE
...
!-- Set top boundary conditions flag
    IF ( bc_aer_t == 'dirichlet' )  THEN
       ibc_aer_t = 0
    ELSEIF ( bc_aer_t == 'neumann' )  THEN
       ibc_aer_t = 1
    ELSEIF ( bc_aer_t == 'initial_gradient' )  THEN
       ibc_aer_t = 2
    ELSEIF ( bc_aer_t == 'nested'  .OR.  bc_aer_t == 'nesting_offline' )  THEN
       ibc_aer_t = 3
    ELSE
...
!-- of the total domain
    IF ( bc_aer_n == 'dirichlet'  .AND.  nyn == ny )  bc_dirichlet_aer_n = .TRUE.
    IF ( bc_aer_n == 'neumann'    .AND.  nyn == ny )  bc_radiation_aer_n = .TRUE.
    IF ( bc_aer_s == 'dirichlet'  .AND.  nys ==  0 )  bc_dirichlet_aer_s = .TRUE.
    IF ( bc_aer_s == 'neumann'    .AND.  nys ==  0 )  bc_radiation_aer_s = .TRUE.
    IF ( bc_aer_l == 'dirichlet'  .AND.  nxl ==  0 )  bc_dirichlet_aer_l = .TRUE.
    IF ( bc_aer_l == 'neumann'    .AND.  nxl ==  0 )  bc_radiation_aer_l = .TRUE.
    IF ( bc_aer_r == 'dirichlet'  .AND.  nxr == nx )  bc_dirichlet_aer_r = .TRUE.
    IF ( bc_aer_r == 'neumann'    .AND.  nxr == nx )  bc_radiation_aer_r = .TRUE.
!
!-- Set the communicator to be used for ghost layer data exchange
!-- 1: cyclic, 2: cyclic along x, 3: cyclic along y, 4: non-cyclic
    IF ( bc_aer_l == 'cyclic' )  THEN
       IF ( bc_aer_s == 'cyclic' )  THEN
          communicator_salsa = 1
       ELSE
          communicator_salsa = 2
       ENDIF
    ELSE
       IF ( bc_aer_s == 'cyclic' )  THEN
          communicator_salsa = 3
       ELSE
          communicator_salsa = 4
       ENDIF
    ENDIF
!
...
26  FORMAT (/'   Boundary conditions for aerosols:', /                                             &
             '      bottom/top:   ',A10,' / ',A10, /                                               &
             '      north/south:  ',A10,' / ',A10, /                                               &
             '      left/right:   ',A10,' / ',A10)
...
!
!-- Large gradients near the boundaries may produce
...
!--    shear-free stable conditions.
       CALL ws_init_flags_scalar( bc_dirichlet_aer_l  .OR.  bc_radiation_aer_l,                    &
...
!------------------------------------------------------------------------------!
 SUBROUTINE salsa_rrd_global_mpi
    ...
    CALL rrd_mpi_io_global_array( 'reglim', reglim )
...
!------------------------------------------------------------------------------!
 SUBROUTINE salsa_wrd_global
    ...
    IF ( TRIM( restart_data_format_output ) == 'fortran_binary' )  THEN
       ...
       CALL wrd_write_string( 'listspec' )
       WRITE ( 14 )  listspec
    ...
    ELSEIF ( restart_data_format_output(1:3) == 'mpi' )  THEN
       ...
       CALL wrd_mpi_io_global_array( 'reglim', reglim )
...
!--             Exchange ghost points
                ...
                ENDIF
!
!--             Apply only horizontal boundary conditions
                CALL salsa_boundary_conditions( horizontal_conditions_only = .TRUE. )
    ...
    IF ( timestep_scheme(1:5) == 'runge' )  THEN
       IF ( ws_scheme_sca )  THEN
          CALL advec_s_ws( salsa_advc_flags_s, i, j, rs, id, flux_s, diss_s, flux_l, diss_l,       &
                           i_omp_start, tn, bc_dirichlet_aer_l  .OR.  bc_radiation_aer_l,          &
                           bc_dirichlet_aer_n  .OR.  bc_radiation_aer_n,                           &
                           bc_dirichlet_aer_r  .OR.  bc_radiation_aer_r,                           &
                           bc_dirichlet_aer_s  .OR.  bc_radiation_aer_s, monotonic_limiter_z )
       ELSE
...
!--       Top boundary conditions:
          ...
          ELSEIF ( ibc_aer_t == 1 )  THEN   ! Neumann
          ...
          ELSEIF ( ibc_aer_t == 2 )  THEN   ! Initial gradient
...
!--       Lateral boundary conditions
          ...
          ELSEIF ( bc_radiation_aer_s )  THEN
          ...
          ELSEIF ( bc_radiation_aer_n )  THEN
          ...
          ELSEIF ( bc_radiation_aer_l )  THEN
          ...
          ELSEIF ( bc_radiation_aer_r )  THEN
       ...
       ELSE
!
!--       Conditions for timelevel t (if called from salsa_exchange_horiz_bounds).
!--       Only horizontal boundary conditions are set here.
          IF ( bc_dirichlet_aer_s )  THEN
       ...
       ENDIF  ! PRESENT( horizontal conditions only )
...
!--          inbalance between number and mass flux
             IF ( index_so4 < 0  .AND.  aero_emission_att%cc_in2mod(1) > 0 )  THEN
                aero_emission%mass_fracs(:,aero_emission_att%cc_in2mod(1)) = 0.0_wp
                aero_emission_att%cc_in2mod(1) = 0
             ENDIF
             IF ( index_oc  < 0  .AND.  aero_emission_att%cc_in2mod(2) > 0 )  THEN
                aero_emission%mass_fracs(:,aero_emission_att%cc_in2mod(2)) = 0.0_wp
                aero_emission_att%cc_in2mod(2) = 0
             ENDIF
             IF ( index_bc  < 0  .AND.  aero_emission_att%cc_in2mod(3) > 0 )  THEN
                aero_emission%mass_fracs(:,aero_emission_att%cc_in2mod(3)) = 0.0_wp
                aero_emission_att%cc_in2mod(3) = 0
             ENDIF
             IF ( index_du  < 0  .AND.  aero_emission_att%cc_in2mod(4) > 0 )  THEN
                aero_emission%mass_fracs(:,aero_emission_att%cc_in2mod(4)) = 0.0_wp
                aero_emission_att%cc_in2mod(4) = 0
             ENDIF
             IF ( index_ss  < 0  .AND.  aero_emission_att%cc_in2mod(5) > 0 )  THEN
                aero_emission%mass_fracs(:,aero_emission_att%cc_in2mod(5)) = 0.0_wp
                aero_emission_att%cc_in2mod(5) = 0
             ENDIF
             IF ( index_no  < 0  .AND.  aero_emission_att%cc_in2mod(6) > 0 )  THEN
                aero_emission%mass_fracs(:,aero_emission_att%cc_in2mod(6)) = 0.0_wp
                aero_emission_att%cc_in2mod(6) = 0
             ENDIF
             IF ( index_nh  < 0  .AND.  aero_emission_att%cc_in2mod(7) > 0 )  THEN
                aero_emission%mass_fracs(:,aero_emission_att%cc_in2mod(7)) = 0.0_wp
                aero_emission_att%cc_in2mod(7) = 0
             ENDIF
!
!--          Then normalise the mass fraction so that SUM = 1
...
!--       Pre-processed:
          ELSEIF ( aero_emission_att%lod == 2 )  THEN
             write(9,*) 'salsa_emission_setup: read from file, update'
             flush(9)
!
...
!--          Obtain time index for current point in time.
             write(9,*) '   obtain time index for current point in time'
             flush(9)
             ...
             write(9,*) 'aero_emission_att%tind: ', aero_emission_att%tind
             flush(9)
             write(9,*) 'aero_emission_att%tind_p: ', aero_emission_att%tind_p
             flush(9)
!
...
!--          Determine the next emission update
             ...
             write(9,*) 'salsa_emission_setup: next_aero_emission_update: ', next_aero_emission_update
...
!--    Determine the next emission update
       ...
       write(9,*) 'next_gas_emission_update: ', next_gas_emission_update
       flush(9)
...
!-- Treat bin-specific outputs separately
    ...
    ELSE
       SELECT CASE ( TRIM( var(7:) ) )
          ...
          CASE ( 'PM0.1', 'PM2.5', 'PM10', 's_BC', 's_DU', 's_NH', 's_NO', 's_OC', 's_SO4', 's_SS' )
             unit = 'kg/m3'
