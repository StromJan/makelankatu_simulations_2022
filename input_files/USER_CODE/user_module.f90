...
! Current revisions:
! -----------------
! Enable reading dynamic and static background conditions for aerosols and define statistical
! regions
! 
! Former revisions:
! -----------------
! $Id: user_module.f90 4535 2020-05-15 12:07:23Z raasch $
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE user_check_parameters
    ...
    USE salsa_mod,                                                                                 &
        ONLY:  init_aerosol_type, init_gases_type, nesting_offline_salsa

!
!-- Here the user may add code to check the validity of further &userpar control parameters or
!-- deduce further quantities.

    IF ( nesting_offline  .AND.  nesting_offline_salsa )  THEN
       salsa_dynamic_background_concentrations = .FALSE.
    ENDIF

    IF ( .NOT. nesting_offline_salsa  .AND. salsa_dynamic_background_concentrations )  THEN
       init_aerosol_type = 1
       init_gases_type = 1
...
!--       Reference
          ...
          rmask(:,:,region) = 0.0_wp
          DO  i = nxl, nxr
             IF ( i >= 458 .AND. i <= 460 )  THEN
                DO  j = nys, nyn
                   IF ( j >= 382 .AND. j <= 384 )  THEN
                      rmask(j,i,region) = 1.0_wp
                   ENDIF
                ENDDO
             ENDIF
...
!--       Next to container
          ...
          rmask(:,:,region) = 0.0_wp
          DO  i = nxl, nxr
             IF ( i >= 313 .AND. i <= 317 )  THEN
                DO  j = nys, nyn
                   IF ( j >= 168 .AND. j <= 172 )  THEN
                      rmask(j,i,region) = 1.0_wp
                   ENDIF
                ENDDO
             ENDIF
...
!--       Reference
          ...
          rmask(:,:,region) = 0.0_wp
          DO  i = nxl, nxr
             IF ( i >= 516 .AND. i <= 520 )  THEN
                DO  j = nys, nyn
                   IF ( j >= 283 .AND. j <= 287 )  THEN
                      rmask(j,i,region) = 1.0_wp
                   ENDIF
                ENDDO
             ENDIF
...
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE user_actions( location )
    ...
    USE salsa_mod,                                                                                 &
        ONLY:  salsa_nest_offl, salsa_nesting_offl_bc, salsa_nesting_offl_input
...
!--          Update boundary conditions
             IF ( salsa_nest_offl%time(salsa_nest_offl%tind_p) <= time_since_reference_point )  THEN
...
!          ENDDO
       ...
       CASE ( 'after_timestep' )
!
!--       Set dynamic boundary conditions
          IF ( salsa  .AND.  salsa_dynamic_background_concentrations )  THEN
             CALL salsa_nesting_offl_bc
          ENDIF
